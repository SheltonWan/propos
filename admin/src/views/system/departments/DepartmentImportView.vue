<template>
  <div class="dept-import">
    <el-page-header @back="goBack">
      <template #content>
        <span class="title">批量导入组织架构</span>
      </template>
    </el-page-header>
    ...
    <el-card v-if="currentStep === 2" class="step-card" shadow="never">
      <el-result :icon="commitIcon" :title="commitTitle" :sub-title="commitSubtitle">
        <template #extra>
          ...
          <el-button type="primary" @click="goBack">返回组织架构</el-button>
        </template>
      </el-result>
    </el-card>
  </div>
...
import { useDepartmentImportStore } from '@/stores'

const store = useDepartmentImportStore()
...
function onDownloadTemplate(): void {
  const headers = ['部门名称', '上级部门名称', '层级', '排序号']
  const rows = [
    ['总公司', '', '1', '0'],
    ['运营部', '总公司', '2', '10'],
    ['租务一组', '运营部', '3', '10'],
