...
6. Flutter / admin 前端严格遵循单向数据流：
   ...
   - 禁止在 Widget/Component 内硬编码 API 路径，统一使用常量文件（Flutter: `api_paths.dart`，Admin: `api_paths.ts`）
...
- Admin 展示：使用 `dayjs(value).format('YYYY-MM-DD')` 转为本地时区显示
...
| 类型 | Flutter 归属文件 | Admin 归属文件 | 示例 |
|------|-----------------|---------------|------|
| 业务规则常量 | `flutter_app/lib/core/constants/business_rules.dart` | `admin/src/constants/business_rules.ts` | 预警天数 90/60/30、逾期节点 1/7/15 天、KPI 满分阈值 95% |
| UI 展示常量 | `flutter_app/lib/core/constants/ui_constants.dart` | `admin/src/constants/ui_constants.ts` | 分页大小、动画时长 |
| API 路径常量 | `flutter_app/lib/core/constants/api_paths.dart` | `admin/src/constants/api_paths.ts` | `/api/contracts`、`/api/invoices` |
