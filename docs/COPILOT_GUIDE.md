...
|------|-----------|----------------|
| **Copilot Chat（Agent 模式）** | 多文件修改、系统设计、跨模块重构 | 架构设计、数据库 Schema 设计、模块骨架生成、文档生成 |
| **Copilot 内联补全** | 单函数/单文件局部补全 | 业务逻辑实现、WALE 计算公式、状态机代码 |
| **Copilot 编辑模式（Edit Mode）** | 定向修改已有文件 | Bug 修复、接口字段调整、权限逻辑更新 |

> **核心原则**：用 Agent 模式做**规划与骨架**，用内联补全做**实现与填充**，用编辑模式做**精确修改**。三种模式形成流水线，不要混用。
...
### 4.2 Agent 模式 vs Chat 模式选择

```
需要修改多个文件 → Agent 模式
需要探索性设计讨论 → Chat 模式（不要让它直接改文件）
需要局部代码补全 → 内联模式（Tab 补全）
需要定向修改某个函数 → Edit 模式（Cmd+I）
```
...
上午（编码时间）：
  → 用内联补全完成昨天设计好的功能实现
  → Copilot Tab 补全细节，不要打断思路去开 Chat

下午（设计时间）：
  → 用 Agent/Chat 模式规划明天的任务
  → 让 AI 评审今天的代码，输出 Code Review 意见
  
遇到 Bug：
  → 先用 Edit 模式（Cmd+I）让 Copilot 就地修复
  → 修复失败再开 Agent 模式加载更多上下文
```
