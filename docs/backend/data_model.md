...
noi_budgets (→ buildings?)                            ← M3 NOI 预算
...
| `noi_budgets` | `building_id` | `buildings` | NOI 预算关联楼标（可选） |
...
### 6.7.1 noi_budgets（NOI 预算）

**v1.7 新增**：录入各楼栋或业态的 NOI 预算值，用于 K07（NOI 达成率）指标计算。匹配规则：取最近一条 `building_id`（或 `property_type`）+ `period_year` + `period_month`（可选）匹配的记录为预算基准。

```sql
CREATE TABLE noi_budgets (
    id             UUID            PRIMARY KEY DEFAULT gen_random_uuid(),
    -- 可按楼栋或业态维度录入，两者均为可选（同时为 NULL 表示全局预算）
    building_id    UUID            REFERENCES buildings(id),
    property_type  property_type,
    -- 预算周期
    period_year    SMALLINT        NOT NULL,  -- 预算年份，如 2026
    period_month   SMALLINT        CHECK (period_month BETWEEN 1 AND 12),  -- NULL 表示年度预算
    -- 预算值
...
> K07 取数逻辑：`actual_noi / budget_noi`，其中 `budget_noi` 取匹配到当前考核周期的最近一条预算记录。实际 NOI 展现实现（received）口径：EGI - OpEx。
...
CREATE TABLE kpi_score_snapshots (
    ...
    snapshot_status   VARCHAR(20) NOT NULL DEFAULT 'draft',  -- 'draft', 'frozen', 'recalculated'
    frozen_at         TIMESTAMPTZ,                           -- 冻结时间（触发申诉窗口 7 日计时起点）
