...
| 维修技工 | 单任务：录入水电读数 | 看到所有财务功能入口，权限不匹配且界面复杂 |
| 楼管巡检员 | 巡检辅助：水电抄表 + 粗略财务概览 | 无专属视图，与维修工混在一起 |
| 只读观察者 | 报表查看：看 NOI/WALE/KPI 概览，零操作 | 无专属视图，混在管理层视图中可能误触操作按钮 |
...
| 方案 | 实现方式 | 优点 | 缺点 |
|------|---------|------|------|
| A：单布局局部 `if` | 同一 JSX 树，分支条件显隐 | 代码量少 | 6 套角色逻辑交织，维护时易误改 |
| **B：整页六视图**（选定） | 6 条 `if/return` 分支，各返回专属 JSX | 每个角色布局独立可维护，视角设计无妥协 | 代码行数多 |
| C：配置驱动渲染 | ROLE_CONFIG 对象 + 单一渲染引擎 | 扩展性强 | 过度设计，6 个角色用 config 对象 ROI 低 |

**选定方案 B**：整页六视图独立渲染。理由：7 个角色中 2 个无财务页访问权（MS/PI 中 PI 可到达但展示极简，MS 不可达），剩余 5 个角色的页面结构差异已大到 header 颜色、组件类型、数据源完全不同，强行共用模板会使每个角色的体验都不精准。
...
### 3.5 楼管巡检视图（property_inspector）

**设计理念**：巡检辅助，水电抄表 + 粗略财务参考

```
Header: 深青渐变 #134e4a→#065f46
页眉:   巡检财务 / 水电 · 报表查看
```
...
1. **水电抄表卡** — 与维修技工相同的录入卡
2. **资产概览迷你卡** — 分业态出租率 3 小卡，供巡检参考
3. **小图标行**：KPI考核（只读）

**关键差异**：比维修技工多一个资产概览，但仍无财务操作权

### 3.6 只读观察视图（report_viewer）

**设计理念**：投资人/审计视角，看所有核心指标，零操作

```
Header: 深紫渐变 #3b0764→#581c87
页眉:   财务概览 / 只读视图
```

**页面结构**：

1. **NOI 汇总卡** — 与管理层相同，但无点击进入编辑
2. **WALE 汇总卡** — 与管理层相同，只读
3. **收入快报卡** — 与管理层相同，只读
4. **KPI 概览卡** — 总分 + 各模块得分概览，只读
5. **逾期账单 Top5** — 仅查看，无“去处理”按钮

**关键差异**：与管理层看板几乎相同，但移除所有操作按钮（无核销、无审批、无导入、无生成账单）
...
└── Finance() 主出口
    ...
    ├── if maintenance_staff → 维修技工视图
    ├── if property_inspector → 楼管巡检视图
    └── if report_viewer → 只读观察视图
```
...
| 维修技工 | `#78350f → #92400e` | 深琥珀（工具） |
| 楼管巡检员 | `#134e4a → #065f46` | 深青（巡检） |
| 只读观察者 | `#3b0764 → #581c87` | 深紫（观察） |
...
| 维修技工视图 | u5 陈师傅（maintenance_staff） | 琥珀 Header + 右上角🟡“2”圆钮 + 极简单卡布局 |
| 楼管巡检视图 | u6 周楼管（property_inspector） | 深青 Header + 资产概览 + 水电抄表 + KPI只读 |
| 只读观察视图 | u7 钱投资（report_viewer） | 深紫 Header + NOI/WALE/收入/KPI 全只读 + 零操作按钮 |
