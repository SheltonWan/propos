...
| `maintenance_staff` | 首页 / 工单 | workorders |
| `property_inspector` | 首页 / 资产（只读）/ 合同（只读、金额脱敏）/ 工单（只读） | assets / contracts（只读）/ workorders（只读） |
| `report_viewer` | 首页 / 资产（只读）/ 合同（只读、PII脱敏）/ 财务（只读） | assets / contracts / finance（全只读） |
...
| `useDashboardStore` | 首页 | `metrics / alerts / loading / error` |
