...
|------|------------------|----------------|
| `super_admin` / `operations_manager` | 全部 5 Tab + 通知 | 全部路由（含 /approvals） |
| `leasing_specialist` | 资产 / 合同 / 工单 / 通知 | assets / contracts / workorders / notifications |
| `finance_staff` | 财务 / 合同（只读）/ 通知 | finance（含 dunning）/ contracts（只读）/ notifications |
...
> 催收管理仅 Admin 端提供，uni-app 端通过通知推送催收提醒。

### 11.1 催收记录列表

**Admin**: `admin/src/views/finance/DunningListView.vue`  
**路由**: `/finance/dunning`  
**Store**: `useDunningStore`  
