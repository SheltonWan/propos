...
Dart SDK 未加入 PATH，通过 Homebrew 安装后手动确认：

```bash
# 如果通过 Homebrew 安装
brew info dart-lang/dart/dart

# 确认 PATH（Apple Silicon Mac）
echo 'export PATH="/opt/homebrew/opt/dart/libexec/bin:$PATH"' >> ~/.zshrc
source ~/.zshrc
```
