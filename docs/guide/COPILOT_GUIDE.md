...
2. [Copilot 四层定制化体系](#二copilot-四层定制化体系)
...
`.github/` 目录已配置完整的四层定制化体系，**无需额外操作**，Copilot 会在合适时机自动加载对应规则。

### 2.1 四层结构概览

```
Layer 1: copilot-instructions.md        ── 全局系统提示（所有对话自动生效）
Layer 2: instructions/*.instructions.md ── 文件类型级自动注入（编辑匹配路径时触发）
Layer 3: prompts/*.prompt.md            ── 手动触发场景提示词（/ 命令调用）
Layer 4: agents/*.agent.md              ── 多步编排 Agent（Agent 选择器调用）
```
...
- 技术栈定义（Dart/Shelf、Flutter + Material 3、Vue 3 + Element Plus、PostgreSQL 15+）
- 核心领域模型与 API 信封格式（`{"data":..., "meta":...}`）
- 代码规范、常量管理规则、Markdown → PDF 工作流
- 文件复杂度超限拆分策略（各层触发阈值与拆分方式）

### 2.3 Layer 2：文件类型级自动注入规则

编辑**匹配路径**的文件时 Copilot 自动加载对应规则，无需手动 `@` 引用：
...
| `flutter.instructions.md` | `flutter_app/lib/**` | BLoC/Cubit 四态状态管理、api_paths 常量、Material 3 主题色、go_router 路由、get_it 依赖注入 |
...
在 Copilot Chat 输入框输入 `/` 可调出提示词列表，所有提示词已内嵌文档引用与约束规则：

| 命令 | 用途 |
|------|------|
| `/backend-module` | 生成后端完整模块（Model → Repository → Service → Controller） |
| `/flutter-page` | 生成 Flutter 页面（实体 + Repository + BLoC/Cubit + Page Widget） |
...
> **使用方式**：在 Copilot Chat 输入 `/` 后选择对应命令，告知目标模块名称即可。

### 2.5 Layer 4：编排 Agent

在 **Agent 模式**的 Agent 选择器中切换对应 Agent：

| Agent | 用途 | 调用方式 |
|-------|------|---------|
| **PropOS Feature Builder** | 全栈模块实现编排，按 Step 1（迁移）→ Step 9（UI）自动执行 9 层 | Agent 选择器选择，输入模块描述 |
| **PropOS Compliance Reviewer** | 只读合规审查，输出分层违规报告（高/中/低优先级） | 由 Feature Builder 完成后自动调用，不出现在选择器 |

> **PropOS Feature Builder 示例**：选择该 Agent → 输入"实现 M2 合同管理模块后端四层" → Agent 自动按序完成 Model → Repository → Service → Controller，最后调用 Compliance Reviewer 审查违规项。
...
2. **输入提示词（复制使用）**：
   ...
   3. Flutter 移动端和 admin PC 端的页面路由结构
   4. RBAC 权限矩阵的代码级实现方案
...
**关键提示词**：
```
实现 cad_import_service.dart，将 .dwg 文件转换为 SVG。
转换链路：
1. Process.run 调用 ODA File Converter CLI（DWG→DXF），
   命令格式：ODAFileConverter <input_dir> <output_dir> ACAD2018 DXF 0 1
2. Process.run 调用 ezdxf draw CLI（DXF→SVG），
   命令格式：ezdxf draw -o <output.svg> <input.dxf>
- 按楼层分割输出（每层一个 SVG 文件）
- 输出文件命名规范：floors/{building_id}/{floor_id}.svg
- 错误时记录日志但不中断批量处理
```
...
**工单模块 AI 策略**（Flutter 移动端）：
```
生成一个 Flutter 工单报修页面（Dart + BLoC），要求：
- 支持三级联动选择器：楼栋 → 楼层 → 单元（从后端 API 异步加载）
- 照片上传：最多 5 张，使用 image_picker + 压缩处理（> 1MB 自动压缩至 800KB）
- 紧急程度选择：Material 3 SegmentedButton 单选，三个选项
- 工单状态用 BLoC/Cubit 管理，@freezed 四态（initial / loading / loaded / error）
- 提交后通过 context.go() 跳转至工单列表页并高亮新建工单
```
...
**AI 使用策略**：
- 先生成二房东账号的 JWT 中间件（需包含 `bound_contract_id` claim，对应 `ctx.boundContractId`）
...
1. 每个端点都从 JWT claim `bound_contract_id` 中提取（对应 `ctx.boundContractId`），而非从请求体接受
...
> **快捷方式**：以下提示词均已保存为 `.github/prompts/*.prompt.md` 文件，在 Copilot Chat 中输入 `/` 即可直接调用，无需手动复制粘贴。每个提示词已内嵌最新文档引用（`API_CONTRACT_v1.7.md`、`RBAC_MATRIX.md`、`PAGE_SPEC_v1.7.md` 等）。下方内容作为调整参考。
...
同时生成对应的 Admin Pinia Store（useWorkOrderStore）用于 PC 端状态管理。
```
...
1. 从注入的 SubleaseSecurityContext 获取 boundContractId（不接受参数传入）
2. 所有 SELECT/UPDATE/DELETE 自动追加 WHERE master_contract_id = $boundContractId
3. INSERT 时自动设置 master_contract_id = $boundContractId
4. 尝试访问其他 boundContractId 的数据时抛出 UnauthorizedException
...
**方式 1（推荐）：使用 PropOS Feature Builder Agent**

该 Agent 在完成所有层次实现后会自动调用 `PropOS Compliance Reviewer`，输出分层违规报告（高/中/低优先级），无需手动触发。

**方式 2：手动运行 `/security-and-test` 提示词**

在 Copilot Chat 输入 `/security-and-test`，然后指定要审查的模块路径。提示词已内嵌以下检查项：

```
审查 [模块路径] 代码，检查：
1. 所有 API 端点是否有 RBAC 权限校验（参照 RBAC_MATRIX.md）
2. 证件号/手机号是否加密存储并脱敏输出
3. 数据库查询是否使用参数化查询（无 ${} 拼接）
4. 二房东相关查询是否有 master_contract_id 行级隔离
5. WALE/NOI/KPI 计算是否有对应单元测试
按高/中/低优先级排列违规项，直接给出修复代码。
```
