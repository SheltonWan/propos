...
> 版本：v1.5（严格对齐 schema additionalProperties）  
> 日期：2026-05-01  
> v1.5 修订摘要：
> - 🔴 `floor_map_updated_at` 不再作为 `FloorMapV2` 顶层字段（与 schema `additionalProperties:false` 冲突），改为通过 HTTP `ETag` 响应头传递；PUT 请求携 `If-Match`，store `ifMatch` 从 header 读取
> - 🔴 §0-E `saveStructures` 不再固定补 `render_mode='vector'`，改为保留 `floors.render_mode` 原值（首次创建才默认 `vector`），避免语义楼层被静默回退
> - 🔴 §0-G `window_detector` 输出不再携 `source` 字段（schema `Window.additionalProperties:false` 且不含 source）
> - 🟡 store `validate` 补两项 schema 限制：structure.label 限长 32；north.x/y 需落在 viewport 内且 rotation_deg ∈ [-180,180]
>
> v1.4 修订摘要：
...
> - 🟢 §0-E 明确：PUT 请求体不携带 `render_mode`，由 Service 层固定写 `'vector'`（语义切换走 PATCH render-mode 单独流程），避免 schema 顶层 required 与 PUT 字段集冲突 ⚠️ **此条已被 v1.5 修订取代**：现改为保留原 `render_mode`
>
> v1.3 修订摘要：
...
| `FLOOR_MAP_VERSION_CONFLICT` | 409 | PUT 请求 `If-Match` 版本号与当前 `floors.floor_map_updated_at` 不一致（并发编辑）；PUT/GET structures 均通过响应头 `ETag` 返回当前版本号 |
...
| `PUT /api/floors/<floorId>/structures` | `assets.write` | `saveStructures`（读取 `If-Match` header） | `200 { data: FloorMap }` + `ETag: <floor_map_updated_at>` 响应头 |
| `GET /api/floors/<floorId>/structures` | `assets.read` | `getConfirmedStructures` | `200 { data: FloorMap }` + `ETag: <floor_map_updated_at>` 响应头 |
...
> 此三字段属于 `floors` 资源元数据（不是 FloorMap body），在 `Floor` 资源响应中平铺返回不冲突。`GET/PUT structures` 响应则额外通过 `ETag` 响应头交付 `floor_map_updated_at`。
...
- 每条窗线输出 `{side, offset, width}`（offset/width 为沿该边的局部坐标，单位 SVG px；字段名与 schema `Window` 严格一致，**不携 `source` 字段**—schema `Window.additionalProperties:false` 且不允许 source；前端 store 校验 `width ≥ 8` 且 `offset + width ≤ sideLen`）
- 上限 100 个/层
...
> 注：`floor_map_updated_at`（乐观锁版本号）**不属于 FloorMap body**—schema 顶层 `additionalProperties:false` 禁止额外字段。后端通过 HTTP `ETag` 响应头返回，前端 PUT 请求通过 `If-Match` 请求头携带。
...
// 获取已确认结构（上次 PUT 保存的数据）—需同时读取响应头 ETag 作为乐观锁版本号
export const getConfirmedStructures = (floorId: string) =>
  apiGet<FloorMapV2>(API_PATHS.floorStructures(floorId), { withResponseHeaders: true });

// 保存审核后的结构（覆盖写，PUT 端点仅接收 source='manual'）
// ifMatch 为上次拉取/保存响应头 ETag 的值，用于乐观锁
...
> `apiGet` / `apiPut` 需扩展以支持返回响应头：在可选 config 中增 `withResponseHeaders?: boolean`，为 `true` 时返回 `{ data, headers }`，默认 false 保持原返回类型不变。`apiPut` 同时接收 `headers?: Record<string,string>` 透传 `If-Match`。
>
> **改造前置校验步骤**（防止破坏现有调用站点）：
> 1. `grep -rn 'apiPut(\|apiGet(' admin/src` 列出所有调用点
> 2. 确认现有调用均不依赖返回头，`withResponseHeaders` 默认为 undefined/false 保持返回类型向后兼容
> 3. 类型重载：当 `withResponseHeaders: true` 时 `apiGet<T>` 返回 `{ data: T; headers: Headers }`，否则返回 `T`
...
const ifMatch      = ref<string | null>(null)       // 乐观锁版本号：从 GET/PUT structures 响应头 `ETag` 读取
...
| Action | 说明 |
|---|---|
| `load(floorId)` | **并行** `Promise.all([getCandidates, getConfirmedStructures])`，二者均允许 404；`draft = confirmed?.data ?? candidates`（保护人工修改不被覆盖）；`baselineSnapshot = deepClone(draft)`；`ifMatch = confirmed?.headers.get('ETag') ?? null` |
| `save(floorId)` | 前端 validate → `putStructures(floorId, draft, ifMatch)` → 用响应 `data` 回填 `confirmed`、用响应头 `ETag` 更新 `ifMatch` → `dirty=false`；捕获 `FLOOR_MAP_VERSION_CONFLICT` 时弹窗提示「数据已被他人更新，是否放弃当前修改并重载？」 |
...
if (map.north) {
  ...
  if (rotation_deg < -180 || rotation_deg > 180) return '指北针 rotation_deg 必须在 [-180, 180]';
}
...
| `save` 成功时 `dirty === false`，`ifMatch` 被刷新为响应头 `ETag`（即新的 `floor_map_updated_at`） | dirty 清除 + 乐观锁推进 |
...
| `validate` structure.label 长度 > 32 | schema label maxLength 限制 |
| `validate` north.x/y 超出 viewport 或 rotation_deg ∉ [-180,180] | NorthArrow 几何 + 取值范围校验 |
...
| 前端 Store 单测 | `pnpm --filter admin test:unit` | 25 个 store 测试用例全绿 |
